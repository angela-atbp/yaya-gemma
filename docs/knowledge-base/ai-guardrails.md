Architectural Guardrails for AI & Lovable
⚠️ Data Separation Warning:
When prompting Lovable or Claude to handle the Weekly Plan or Shared Grocery List, design the database schema to support an eventual family_id or household_id abstraction, even if everything currently filters by a single user_id. This prevents severe structural rewrites when we activate Phase 2.
